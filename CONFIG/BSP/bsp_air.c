...
//	struct  //遥控原始数据，8通道
//	{
//	 uint16_t roll;			//ROCK_R_X
//	 uint16_t pitch;		//ROCK_R_Y
//	 uint16_t thr;          //ROCK_L_Y
//	 uint16_t yaw;          //ROCK_L_X
//	 uint16_t AUX1;         AIR_R_SHORT
//	 uint16_t AUX2;         AIR_R_LONG
//	 uint16_t AUX3;         AIR_L_LONG
//	 uint16_t AUX4;         AIR_L_SHORT
//	}Remote; 

//}Air_Contorl;

void bsp_Air_Init(void){		//脉冲捕获//最高支持8通道
...
//	Feed_Rc_Dog(1);//RC看门狗清零，表示遥控信号正常
	...
	if(TIM1->SR & TIM_IT_CC1)  //  SR：TIM status register
	{
		TIM1->SR = ~TIM_IT_CC1;//TIM_ClearITPendingBit(TIM3, TIM_IT_CC1);  如果中断了，改成没有中断
